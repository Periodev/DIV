...
func _draw() -> void:
	...
	var s := _cell_size()
	var scale := float(s) / BASE_CELL_SIZE

	for y in gs:
		for x in gs:
			var pos := Vector2i(x, y)
			var tt: int = branch_state.terrain.get(pos, Enums.TerrainType.FLOOR)
			var rect := Rect2(float(x * s), float(y * s), float(s), float(s))
			_draw_terrain_cell(rect, tt, pos, scale)
...
func _draw_terrain_cell(rect: Rect2, terrain_type: int, pos: Vector2i, scale: float) -> void:
	match terrain_type:
		Enums.TerrainType.WALL:
			draw_rect(rect, COLOR_WALL)
		Enums.TerrainType.GOAL:
			draw_rect(rect, COLOR_GOAL)
			_draw_center_text("Goal", rect.get_center(), int(14 * scale), COLOR_TEXT)
		Enums.TerrainType.SWITCH:
			var active := branch_state.switch_activated(pos)
		...
		Enums.TerrainType.BRANCH1, Enums.TerrainType.BRANCH2, Enums.TerrainType.BRANCH3, Enums.TerrainType.BRANCH4:
			...
			_draw_branch_marker(rect.get_center(), terrain_type, scale)
		_:
			draw_rect(rect, COLOR_FLOOR)


func _draw_branch_marker(center: Vector2, terrain_type: int, scale: float) -> void:
	var uses_map := {
	...
	var base_radius: float = maxf(2.0, 4.0 * scale)
	var line_width: float = maxf(1.0, 2.0 * scale)
...
func _draw_boxes(cell_size: int, scale: float) -> void:
	for e in branch_state.entities:
		var ent := e as Entity
		if ent == null or ent.uid == 0:
			continue
		var pad_base := 15 if ent.z == -1 else 9
		var pad := int(pad_base * scale)
		var rect := Rect2(
		...
		draw_rect(rect, fill)
		draw_rect(rect, COLOR_TEXT, false, max(1.0, 2.0 * scale))
		_draw_center_text(str(ent.uid), rect.get_center(), int(14 * scale), COLOR_TEXT)
...
func _draw_grid(cell_size: int, scale: float) -> void:
	var gs := branch_state.grid_size
	var thick: float = maxf(1.0, 1.0 * scale)
	for y in gs:
		for x in gs:
			var pos := Vector2i(x, y)
			var tt: int = branch_state.terrain.get(pos, Enums.TerrainType.FLOOR)
			...
			if tt == Enums.TerrainType.SWITCH:
				var active := branch_state.switch_activated(pos)
				var inset: int = maxi(1, int(3 * scale))
				var c := COLOR_SWITCH_ON_B if active else COLOR_SWITCH_INNER
				var inner_rect := Rect2(
				...
				draw_rect(inner_rect, c, false, max(1.0, 3.0 * scale))
			else:
